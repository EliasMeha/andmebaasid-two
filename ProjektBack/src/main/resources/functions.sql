...
CREATE OR REPLACE FUNCTION f_kontrolli_juhataja_saab_sisse_logida(p_kasutajanimi text, p_parool text)
    RETURNS boolean AS $$
DECLARE
    rslt boolean;
BEGIN
    SELECT INTO rslt (parool = laiendused.crypt(p_parool, parool))
    FROM kasutajakonto k
             JOIN tootaja_rolli_omamine t USING (isik_id)
             JOIN isik i USING (isik_id)
             JOIN tootaja too USING (isik_id)
    WHERE i.e_meil= p_kasutajanimi
      AND t.tootaja_roll_kood = 1
      AND LOCALTIMESTAMP(0) >= (t.alguse_aeg)::timestamp without time zone
      AND LOCALTIMESTAMP(0) <= t.lopu_aeg
      AND i.isiku_seisundi_liik_kood = 1
      AND k.on_aktiivne = true
      AND ((too.tootaja_seisundi_liik_kood = 1) OR (too.tootaja_seisundi_liik_kood = 2) OR (too.tootaja_seisundi_liik_kood = 4));
