CREATE OR REPLACE VIEW laadimispunkt_koondaruanne WITH (
  security_barrier=true
)
AS
SELECT s.laadimispunkti_seisundi_liik_kood, UPPER(s.laadimispunkti_seisundi_liik_nimetus),
       count(l.laadimispunkti_kood) AS laadimispunktide_arv
FROM laadimispunkt AS l RIGHT JOIN laadimispunkti_seisundi_liik AS s USING (laadimispunkti_seisundi_liik_kood)
...
CREATE OR REPLACE VIEW laadimispunkt_koik WITH (
  security_barrier=true
)
    AS
    SELECT l.laadimispunkti_kood, l.laadimispunkti_nimetus, l.pikkuskraad, l.laiuskraad, lsl.laadimispunkti_seisundi_liik_nimetus
    FROM Laadimispunkt AS l INNER JOIN laadimispunkti_seisundi_liik AS lsl USING (laadimispunkti_seisundi_liik_kood);
...
CREATE OR REPLACE VIEW laadimispunkt_detailid WITH (
  security_barrier=true
)
    AS
    SELECT l.laadimispunkti_kood, l.laadimispunkti_nimetus, l.laiuskraad, l.pikkuskraad, l.reg_aeg,
           lsl.laadimispunkti_seisundi_liik_nimetus, lt.laadimispunkti_tyyp_nimetus, lt.kwh, i.eesnimi, i.perenimi,
           i.e_meil
...
CREATE OR REPLACE VIEW laadimispunkt_kategooriad WITH (
    security_barrier=true
)
    AS
    SELECT lkt.laadimispunkti_kategooria_tyyp_nimetus, lk.laadimispunkti_kategooria_nimetus,
           lk.laadimispunkti_kategooria_kood, lko.laadimispunkti_kood
...
CREATE OR REPLACE VIEW laadimispunkt_saab_lopetada WITH (
    security_barrier=true
)
    AS
    SELECT l.laadimispunkti_kood, l.laadimispunkti_nimetus, l.laadimispunkti_seisundi_liik_kood, lsk.laadimispunkti_seisundi_liik_nimetus
    FROM laadimispunkt AS l
        LEFT JOIN laadimispunkti_seisundi_liik AS lsk USING (laadimispunkti_seisundi_liik_kood)
    WHERE l.laadimispunkti_seisundi_liik_kood = 3 OR l.laadimispunkti_seisundi_liik_kood=2;
